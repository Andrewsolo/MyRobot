...
void Task_MotionAutomat(void){
	...
	if (millis() >= motionautomat_Timer) {
		...
		if(MA_commands_head != MA_commands_tail){	// ≈сли в буфере есть команды
			...
			switch(cmd){
				case MA_COMMAND_FORWARD:
					barrierdetect_enable();
					motors_go_forward();				
					break;
				
				case MA_COMMAND_FORWARD_AND_LEFT:
					barrierdetect_enable();
					motors_go_forward_and_left();
					break;
				
				case MA_COMMAND_FORWARD_AND_RIGHT:
					barrierdetect_enable();
					motors_go_forward_and_right();
					break;
				
				case MA_COMMAND_BACKWARD:
					motors_go_backward();
					break;
				
				case MA_COMMAND_BACKWARD_AND_LEFT:
					motors_go_backward_and_left();
					break;
				
				case MA_COMMAND_BACKWARD_AND_RIGHT:
					motors_go_backward_and_right();
					break;
				
				case MA_COMMAND_LEFT:
					motors_go_left();
					break;
				
				case MA_COMMAND_RIGHT:
					motors_go_right();
					break;
				
				case MA_COMMAND_STOP:
				...
				case MA_COMMAND_IDLE:
						break;
			}
			if(cmd>=MA_COMMAND_SPEED_0 && cmd<=MA_COMMAND_SPEED_11){					
				uint8_t newspeed;
